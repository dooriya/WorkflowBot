...
export namespace AdaptiveCards {
    ...
    export function setRefreshAction(card: IAdaptiveCard, refreshCardAction: RefreshCardAction) {
        card.refresh = {
            action: {
                type: "Action.Execute",
                title: "Submit",
                verb: refreshCardAction.verb,
                data: refreshCardAction.data
            },
            userIds: refreshCardAction.userIds
        }
    }
    ...
    export function declareWithoutData(template: any) {
        ...
        return {
            template,
            refresh(verb: string, userIds: string[]) {
                return withoutData.refresh(verb, userIds);
            },
            render(): Schema {
